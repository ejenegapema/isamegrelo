...
// Build the request string for your original parser
$request = "tema, $date_2, $date_3, $station";

// ----------------- Original fetch_tema.php code -----------------
if (stripos($request, 'tema') === 0) {
    $parts = array_map('trim', explode(',', $request));
    $date_2 = $parts[1]; // start date
    $date_3 = $parts[2]; // end date
    $station = $parts[3]; // station ID
    ...
    for ($y = $startYear; $y <= $endYear; $y++) {
        ...
        // Detect temperature anomaly column dynamically
        $headerRow = $table->getElementsByTagName('tr')->item(1);
        $colIndex = 4; // default fallback
        if ($headerRow) {
            foreach ($headerRow->getElementsByTagName('td') as $i => $td) {
                $text = trim($td->textContent);
                if (stripos($text, 'откл') !== false || stripos($text, 'Anomaly') !== false) {
                    $colIndex = $i;
                    break;
                }
            }
        ...
        foreach ($table->getElementsByTagName('tr') as $ri => $tr) {
            ...
            $temaCell = trim($cells->item($colIndex)->textContent);
            if ($dateCell === '' || $temaCell === '') continue;

            $temaVal = str_replace(',', '.', preg_replace('/[^\d\-,\.]/u', '', $temaCell));
            if ($temaVal === '' || $temaVal === '-') continue;

            $dates[] = $dateCell;
            $tema[]  = floatval($temaVal);
        }
    }

    // Slice arrays by requested date range
...
echo json_encode(['error' => 'request not recognized']);
